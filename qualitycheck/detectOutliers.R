...
prep.interactions <- function(for_wilc) {
  ...
  # map drug codes to full names
  for_wilc$Drugname <- plyr::mapvalues(for_wilc$Drug, from = druglegend$code,
                                   to = druglegend$Drug)
  for_wilc$donorname <- plyr::mapvalues(for_wilc$donor, from = druglegend$code,
                                    to = druglegend$Drug)
  for_wilc$comb <- map2_chr(for_wilc$Drugname, for_wilc$donorname, ~ paste(sort(c(.x, .y)), collapse = "_"))
...
pdf(paste0(figdir, "eps-vectors.pdf"),
    width = 11, height = 10,
    onefile = T)
for(i in 1:length(combs)) {
  ...
  p = pheatmap(eps_heatmat,
               ...
               breaks = myBreaks,
               main = paste(combs[i], 
                            paste0(type, ";"), "efsize(eps) =", 
                            format(eps.efsize, digits = 2),
