...
home_dir = "~/Documents/embl/screen_K/screenK1_annotated_reads/batch2/"
...
stats_dir = "~/Documents/embl/screen_K/screenK1_stats/batch2/"
...
annotated = lapply(annotated, function(x) subset(x, x$Time_points <= 20))
annotated = lapply(annotated, function(x) subset(x, !x$Time_points %in% c(2,5,9,13,17,21,25)))
...
for (drug in drvec.red) {
  ...
  p = ggplot(plyr::ldply(recip, 'data.frame'), 
             aes(x=Time_points, y=OD, colour = as.factor(position), shape = as.factor(run))) +
    geom_point() +
...
for (r in unique(recip_df$run)) {
  p = ggplot(recip_df %>%
    ...
    ggtitle(paste("cells control wells - run", r,
                  "(not shuffled) - by donor concentration")) +
    theme_bw() +
    scale_y_continuous(limits = c(min(unlist(lapply(annotated,function(df) min(df$OD_no1st)))),
                                  max(unlist(lapply(annotated,function(df) max(df$OD_no1st)))))) +
...
for (i in seq_len(pages)) {
  page <- ggplot(annotated_unl, aes(x = Time_points, y = med_plate_bug, colour=run)) + 
    geom_point() + geom_line() +
...
pdf(paste0(figdir, "hist_BUG_wells.pdf"), width=30, height=30)

time6 = subset(annotated_unl, Time_points == 18)
ordered.data <- time6[order(time6$run),]
time6$plate_name <- factor(time6$plate_name, levels = unique(ordered.data$plate_name))

ggplot(filter(time6, Drug == "BUG") , aes(x = plate_name, y = mean_plate_bug, fill = factor(donor), colour = factor(plate_name))) +
  geom_bar(position=position_dodge(),stat = "identity") +
  geom_errorbar(aes(ymin = min_plate_bug, ymax = max_plate_bug),
                colour="black", size = 1, width=.3, position=position_dodge(.9)) +
  labs(x = "BUG controls per plate", y = "Mean, min and max of OD at tp = 6", fill = "plate") +
...
for (r in unique(recip_df$run)) {
  p = ggplot(recip_df %>%
             ...
             aes(x=Time_points, y=OD_no1st, group=interaction(plate_name,Well), colour = factor(donor_conc))) +
    geom_point() + geom_line() +
    ggtitle(paste("medium control wells - run", r, "(not shuffled) - by donor concentration")) + 
    theme_bw() +
    scale_y_continuous(limits = c(min(unlist(lapply(annotated,function(df) min(df$OD_no1st)))),
                                  max(unlist(lapply(annotated,function(df) max(df$OD_no1st)))))) +
    geom_dl(aes(label = position), method = list(dl.trans(x = x + 0.2), "last.points", 'last.bumpup', cex = 0.9, rot=c(30))) +
    guides(colour=guide_legend(title="Donor conc")) + 
    theme(plot.title = element_text(hjust = 0.5, size =22))
...
#------------------plot heatmap for 18th time point
...
pdf(paste0(figdir, "heatmap_plates_18thtp_ODno1st.pdf"), width=26, height=18)
...
#raw OD
pdf(paste0(figdir,"heatmap_plates_18thtp_ODraw.pdf"), width=26, height=18)
pages = ceiling(length(unique(time6$plate_name))/9)
...
#it printed the same r in all plots
...
onlyrs = allrep_unl[ , c("Time_points", "OD_r1", "OD_r2", "OD_no1st_r1", "OD_no1st_r2", "AUC_well_ODno1st_r1",
                         "AUC_well_ODno1st_r2", "AUC_norm_bug_r1", "AUC_norm_bug_r2", "slope", "plate_name", "position",
                         "run", "batch", "donor", "donor_conc", "Drug", "Drug_Flag_Conc", "flag_fixed")]
#get r per plate
onlyrs$ODr_corr = NULL
## NEW
onlyrs = unique(onlyrs)
onlyrs = subset(onlyrs, Time_points == 18)

rcorr = onlyrs %>% dplyr::group_by(plate_name) %>%
  dplyr::summarise(ODr_corr=cor(AUC_norm_bug_r1,AUC_norm_bug_r2)) %>% ungroup()
...
plot_reps <- function (df, plot_dir) {
  ...
  pages = ceiling(length(unique(df$plate_name)) / 9)
  ...
  for (i in seq_len(pages)){
    page <- ggplot(df, aes(x=AUC_norm_bug_r1, y=AUC_norm_bug_r2)) +
      geom_point(aes(size = 0.8, colour = as.factor(flag_fixed)), alpha = 0.7) +
      geom_text_repel(data=subset(df, abs(AUC_norm_bug_r1 - AUC_norm_bug_r2) > 0.25),
                      aes(label=Drug, colour = as.factor(flag_fixed)), size = 8) +
      facet_wrap_paginate(run ~ plate_name, ncol= 3, nrow = 3, page = i, scales='free') +
      ...
      ggpubr::stat_cor(method = "pearson", label.y = max(df$AUC_norm_bug_r2) - 0.05, 
                       label.x = min(df$AUC_norm_bug_r1), size = 14) +
      ...
      labs(title = "Replicate correlation", x="AUCnorm_bug_rob rep1", y="AUCnorm_bug_rob rep2", colour = "Drug") +
