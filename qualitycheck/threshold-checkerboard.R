## script for plotting checkerboards near the threshold
...
strong.inter = mutate(strong.inter, padj = p.adjust(pval, method = "BH"))
# subset only to interactions passing the strong threshold
strong.inter = filter(strong.inter, abs(medpbliss) >= 0.05 & padj <= 0.05)

# only interactions close to the threshold
strong.inter = filter(strong.inter, abs(medpbliss) < 0.07)
strong.inter = arrange(strong.inter, padj, desc(abs(medpbliss)))
strong.inter = mutate(strong.inter,  rec = gsub("(.+)(_.*)", "\\1", comb),
                      don = gsub("(.*_)(.+)", "\\2" , comb))
...
pdf(paste0(figdir, "thresh", argv[2], "-checkerboard.pdf"),
  ...
  ch.plot = ggplot(cbd.df, aes(x = factor(recconc,
    ...
    labs(x = "",
         ...
         title = paste("Combination checkerboard", d1, "+", d2,
                       ...
                       ", noise=", format(strong.inter$noise[i], digits = 2))) + 
