...
# here obtain single-drug fitness using optimization (minimize sum of squared residuals)
...
strong.inter = arrange(strong.inter, padj, desc(abs(efsize)))
...
donconc = dplyr::mutate(donconc, donor = plyr::mapvalues(donor,
                                                         from = druglegend$code,
                                                         to = druglegend$Drug))
runmap = distinct(alldata_df, donor, donor_conc, batch, biol_rep, run)
donfit = inner_join(dplyr::mutate(donfit,
                         batch = as.integer(as.character(batch)),
                         donor_conc = as.integer(as.character(donor_conc)),
                         biol_rep = as.integer(as.character(biol_rep))), runmap,
           by=c("donor", "donor_conc", "batch", "biol_rep"))

donfit = left_join(mutate(donfit, donor_conc = as.integer(donor_conc)), donconc)
if(any(is.na(donfit$donor_real_conc))) {
  warning("Some plates with missing real donor concentrations")
  message("Using donor_conc as donor_real_conc")
  donfit = dplyr::mutate(donfit, 
                         donor_real_conc = ifelse(is.na(donor_real_conc), 
                                                  donor_conc, donor_real_conc))
  donconc = dplyr::distinct(donfit,
                            donor_real_conc, run, donor, donor_conc)
}

# save the data for Shiny app
screendata = list(fitdata = alldata_clean,
                  tsb = recfit,
                  donfit = donfit)
saveRDS(screendata, file = paste0(argv[1], "/forshiny/fitness.rds"))
...
pdf(paste0(figdir, "checkerboard.pdf"),
  ...
  ch.plot = ggplot(cbd.df, aes(x = factor(recconc,
    ...
    labs(x = "",
         ...
         title = paste("Combination checkerboard", d1, "+", d2,
                       ...
                       ", efsize=", format(strong.inter$efsize[i], digits = 2))) + 
...
# 
# 
# strong.inter = inner_join(strong.inter, eps)
# 
...
#                                       eps.q3))
# 
# strong.inter = arrange(strong.inter, padj, desc(efsize))
# 
# pdf(paste0(figdir, "checkerboard-restricted.pdf"),
#     onefile = T, width = 12, height = 10)
# for (i in 1:nrow(strong.inter)) {
#   d1 = strong.inter$don[i]
#   d2 = strong.inter$rec[i]
...
#            ifelse(max(chboard.av$ODnorm_bug_rob) < 2, max(chboard.av$ODnorm_bug_rob),
#                   2))
#   esize = paste0(ifelse(strong.inter$type[i] == "synergy", "Q1 = -", "Q3 = "),
#                  format(strong.inter$efsize[i], digits=2))
#   
#   cbd.df = inner_join(chboard.av, only.compl)
#   cbd.df = mutate(cbd.df, biol_rep = paste("biol_rep", biol_rep))
#   
